...
export function ApiKeyInput({ onApiKeyChange }: ApiKeyInputProps) {
  ...
  // 로컬 스토리지 및 환경변수에서 API 키 로드
  useEffect(() => {
    const envGeminiKey = import.meta.env.VITE_GEMINI_API_KEY || '';
    const envNanoBananaKey = import.meta.env.VITE_NANO_BANANA_API_KEY || '';

    const savedGeminiKey = localStorage.getItem('gemini_api_key') || envGeminiKey;
    const savedNanoBananaKey = localStorage.getItem('nano_banana_api_key') || envNanoBananaKey;
